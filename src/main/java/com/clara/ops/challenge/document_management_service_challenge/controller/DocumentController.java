...
public class DocumentController {
  ...
  @Operation(
      ...
      description =
          "Search documents by user ID, document name, and tags. Returns paginated results ordered"
              + " by creation date descending.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Successfully retrieved paginated documents"),
        @ApiResponse(responseCode = "400", description = "Invalid request parameters"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
      })
  @GetMapping("/search")
  public PaginatedDocumentSearch searchDocuments(
      @Parameter(description = "User ID to filter by") @RequestParam(required = false)
          String userId,
      @Parameter(description = "Partial document name to search") @RequestParam(required = false)
          String documentName,
      @Parameter(description = "List of tags to filter by") @RequestParam(required = false)
          List<String> tags,
      @Parameter(description = "Page number (starting from 0)") @RequestParam(defaultValue = "0")
          int page,
      @Parameter(description = "Number of items per page") @RequestParam(defaultValue = "10")
          int size) {
    DocumentSearchCriteria criteria =
        new DocumentSearchCriteria(userId, documentName, tags, page, size);
