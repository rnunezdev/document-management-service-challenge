...
class DocumentControllerTest {
    ...
    void shouldUploadDocumentSuccessfully() throws Exception {
        DocumentUploadRequestDTO metadata = DocumentUploadRequestDTO.builder()
                .user("test-user")
                .tags(List.of("invoice", "2025"))
                .documentName("test.pdf").build();
    ...
    void shouldReturnPaginatedDocuments() throws Exception {
        List<DocumentDto> docs = List.of(new DocumentDto(1L, "test-user", "doc.pdf", List.of("a"), 123L, "application/pdf", null));
        Metadata metadata = new Metadata(0, 10, 1, 1, 1);
        PaginatedDocumentSearch response = new PaginatedDocumentSearch(metadata, docs);

        when(documentService.searchDocumentsWithMetadata(any(DocumentSearchCriteria.class))).thenReturn(response);
        ...
        PaginatedDocumentSearch parsed = objectMapper.readValue(responseJson, PaginatedDocumentSearch.class);
